...
function reset(){ship={x:W/2-16,y:H-45,w:32,h:28};rocks=[];score=0;frame=0;dead=false;left=false;right=false;stat.textContent='PUNTOS 0'}
...
function hold(id,key){var b=document.getElementById(id);b.onpointerdown=function(e){e.preventDefault();if(dead)reset();if(key==='left')left=true;else right=true};['pointerup','pointerleave','pointercancel'].forEach(function(n){b.addEventListener(n,function(){if(key==='left')left=false;else right=false})})}hold('left','left');hold('right','right');
